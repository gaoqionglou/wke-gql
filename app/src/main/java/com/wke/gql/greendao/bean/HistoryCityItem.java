...
//            "isHot": false
    ...
    @Generated(hash = 140440737)
    public HistoryCityItem(Long airportId, String airportCode,
                           String airportCnName, String airportEnName, String airportPinyin,
                           String airportPinyinShort, String cityCode, String cityCnName,
                           String cityEnName, String cityPinyin, String cityPinyinShort,
                           String countryCnName, String countryEnName, String continentCnName,
                           String continentEnName, String longitude, String latitude,
                           String isDomestic, String isHot, String airportNameSimple,
    ...
    @Generated(hash = 414656449)
    public HistoryCityItem() {
    }
    ...
    public static void addHistoryCity(CityItem item) {
        ...
        // 若历史机场航线记录数大于等于5条，则先把历史最久的记录删除，保证记录数少于4条
        deleteLatestRecords(item.isDomestic, 1);

        insert(historyCity);
    }

    public static void deleteHistoryCity(HistoryCityItem historyCity) {
        queryBuilder(HistoryCityItem.class)
                .where(HistoryCityItemDao.Properties.CityCnName.eq(historyCity.cityCnName)
                        , HistoryCityItemDao.Properties.CityEnName.eq(historyCity.cityEnName))
                .buildDelete().executeDeleteWithoutDetachingEntities();
    }

    private static void deleteLatestRecords(String isDomestic, int latestRecordsNum) {
        QueryBuilder<HistoryCityItem> qb = getInstance().getDaoSession().getHistoryCityItemDao().queryBuilder();
        while (qb.where(HistoryCityItemDao.Properties.IsDomestic.eq(isDomestic)).count() > 5 - latestRecordsNum) {
            HistoryCityItem hc = qb.where(HistoryCityItemDao.Properties.IsDomestic.eq(isDomestic)).list().get(0);
            delete(hc);
        }
    }
